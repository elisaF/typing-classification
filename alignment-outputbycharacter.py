...
class ErrorAligner:
    ...
    def outputforanalysis(self, align1, align2, seq2, itempartid, times, typedseq):
        ...
        while m < len(align1):
            m1 = len(align1)-1-m
            if align1[m1] != align2[m1]:
                errorfreeword = u"0"
                
            if align1[m1] != "^":
                toprint = u"\""
                toprint = toprint + itempartid
                toprint = toprint + u"\"\t\""
                toprint = toprint + align1[m1]
                toprint = toprint + u"\"\t\""
                toprint = toprint + align2[m1]
                #toprint = toprint + u"\"\t\""
                #toprint = toprint + unichr(m2)
                toprint = toprint + u"\"\t\""
                toprint = toprint + errorfreeword
                toprint = toprint + u"\"\t\""
                toprint = toprint + unicode(times[m2]) + u"\"\n"
    ...
    def parse_errors(self):
        # write header
        header = "\"PARTID\"\t\"SENTID\"\t\"Typed\"\t\"Intended\"\t\"ERRORFREE\"\t\"IKI\"\n"
